<?php
ob_start();
define('API_KEY','240831035:AAEkOJdoTSiP3kKaKefrz1RRWOKiaCvphfE');
$the_admin_id = 46853604;

file_put_contents("count",file_get_contents("count")+1);

$ad_btn='ساخت دکمه شیشه ای';
$ad_url='https://telegram.me/inlinekeys_bot';
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@inlinekeys_bot getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
<pre>@inlinekeys_bot d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
قبل از هر کاری اول باید بدانید این via inlinekeys_bot چیست !
...
<code>@inlinekeys_bot getlist-XXXXX </code>
...
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@inlinekeys_bot getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@inlinekeys_bot getlist-','',$code));
...
به ربات ما سر بزنید
https://telegram.me/inlinekeys_bot
                            "
                        ]);
                    }else{
                        $newList = [];
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@inlinekeys_bot getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@inlinekeys_bot getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                    ]);
//                    makeHTTPRequest('sendMessage',[
//                        'text'=>'@inlinekeys_bot getbanner-'.getCurrentMakeID($userID),
//                        'chat_id'=>$userID
//                    ]);
//                }break;
                ...
                case '7':{
                    ...
                    if($image_id === false){
                        ...
                        $fule = 'https://api.telegram.org/file/bot240831035:AAEkOJdoTSiP3kKaKefrz1RRWOKiaCvphfE/'.trim($imag->result->file_path,"\\/");
